...
use crate::{sync::atomic::AtomicPtr, Domain, HazPtrObject, HazPtrRecord};

pub struct HazardPointerArray<'domain, F, const N: usize> {
    // ManuallyDrop is required to prevent the HazardPointer from
    // reclaiming itself, since HazardPointerArray has it's own drop
    // implementation with an optimized reclaim for all hazard
    // pointers
    haz_ptrs: [ManuallyDrop<HazardPointer<'domain, F>>; N],
}

impl<'domain, F, const N: usize> HazardPointerArray<'domain, F, N> {
    pub fn hazard_pointers<'array>(
        &'array mut self,
    ) -> [&'array mut HazardPointer<'domain, F>; N] {
        // replace with `self.haz_ptrs.each_mut().map(|v| &mut **v)` when
        // each_mut stabilizes

        let mut out: [MaybeUninit<&'array mut HazardPointer<'domain, F>>;
            N] = [(); N].map(|_| MaybeUninit::uninit());
        ...
        // # Safety
        //
        // We have initialized every element of the array with our for loop
        // above
    ...
    /// reference, or null. Caller must also guarantee that the value
    /// behind the `AtomicPtr` will only be deallocated through calls
    /// to [`HazPtrObject::retire`] on the same [`Domain`] as this holder
    /// is associated with.
        ...
        F: 'static,
    {
        ...
        for (i, (hazptr, src)) in
            self.haz_ptrs.iter_mut().zip(&mut sources).enumerate()
        {
...
impl<'domain, F, const N: usize> Drop
    for HazardPointerArray<'domain, F, N>
{
    fn drop(&mut self) {
        self.reset_protection();
        let domain = self.haz_ptrs[0].domain;
        // replace with `self.haz_ptrs.each_ref().map(|v| v.hazard)` when
        // each_ref stabilizes
...
impl HazardPointer<'static, crate::Global> {
    ...
    pub fn make_many_global<const N: usize>(
    ) -> HazardPointerArray<'static, crate::Global, N> {
...
impl<'domain, F> HazardPointer<'domain, F> {
    ...
    pub fn make_many_in_domain<const N: usize>(
        ...
        let haz_ptrs = domain.acquire_many::<N>().map(|hazard| {
            ManuallyDrop::new(HazardPointer { hazard, domain })
        });
        ...
        F: 'static,
    {
        ...
        loop {
            ...
            match unsafe { self.try_protect(ptr, src) } {
                | Ok(None) => break None,
                // Safety:
                // This is needed to workaround a bug in the borrow
                // checker. See:
