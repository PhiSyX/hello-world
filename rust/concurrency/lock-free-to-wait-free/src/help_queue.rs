  ...
  T: Copy + PartialEq + Eq,
{
  pub(crate) fn new() -> Self {
    ...
    // TODO: Once consts can depend on T, make this constant instead of going
    // via Vec
  ...
  pub(crate) fn try_remove_front(&self, front: T) -> Result<(), ()> {
    ...
    if next.is_null()
      || (unsafe { &*next }.value.expect("not a sentinel node")) != front
    {
      return Err(());
    }
  ...
  fn help_enq(&self, id: usize, phase: u64) {
    while self.is_still_pending(id, phase) {
      ...
      // We know we have a consistent (tail, tail.next) pair, and that it likely
      // still needs to be updated, so let's try to actually execute the
      // to-be-enqueued node from the enqueuing thread's descriptor.
...
// A wait-free queue.
pub(crate) type HelpQueue<LF, const N: usize> =
  WaitFreeHelpQueue<*const OperationRecordBox<LF>, N>;
