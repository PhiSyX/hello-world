...
use hazard_pointers::*;
use loom::{sync::atomic::AtomicPtr, thread};
...
fn acquires_multiple() {
    loom::model(|| {
        ...
        let t1 = thread::spawn(move || {
            let mut hazptr_array =
                HazardPointer::make_many_in_domain(&domain);

            let [my_x, my_y] =
                unsafe { hazptr_array.protect_all([&x1, &y1]) };
    ...
    loom::model(|| {
        ...
        let old = x.swap(
            Box::into_raw(Box::new(
                HazPtrObjectWrapper::with_global_domain((
                    9001, drops_9001,
                )),
            )),
