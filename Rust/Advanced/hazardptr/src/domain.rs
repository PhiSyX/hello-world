...
use crate::{
    sync::atomic::{AtomicIsize, AtomicPtr, AtomicU64, AtomicUsize},
    Deleter, HazPtrRecord, Reclaim,
};

#[cfg(feature = "std")]
const SYNC_TIME_PERIOD: u64 =
    std::time::Duration::from_nanos(2000000000).as_nanos() as u64;
...
impl<F> Domain<F> {
    ...
    pub(crate) fn acquire_many<const N: usize>(
        &self,
    ) -> [&HazPtrRecord; N] {
        debug_assert!(N >= 1);
        ...
        [(); N].map(|_| {
            if !head.is_null() {
                ...
                if !tail.is_null() {
                    unsafe { &*tail }.available_next.store(
    ...
    pub(crate) fn release_many<const N: usize>(
        &self,
        recs: [&HazPtrRecord; N],
    ) {
        let head = recs[0];
    ...
    fn try_acquire_available<const N: usize>(
        &self,
        ...
        loop {
            let avail =
                self.hazptrs.head_available.load(Ordering::Acquire);
            ...
            if (avail as usize & LOCK_BIT) == 0 {
                ...
                //
                // XXX: This could be a fetch_or and allow progress even if
                // there's a new (but unlocked) head.
                if self
                    .hazptrs
                    .head_available
                    ...
                    .is_ok()
                {
                    ...
                    let (rec, n) = unsafe {
                        self.try_acquire_available_locked::<N>(avail)
                    };
    ...
    unsafe fn try_acquire_available_locked<const N: usize>(
        ...
        let mut n = 1;
        let mut next =
            unsafe { &*tail }.available_next.load(Ordering::Relaxed);
    ...
    pub(crate) fn acquire_new(&self) -> &HazPtrRecord {
        ...
        loop {
            ...
            match self.hazptrs.head.compare_exchange_weak(
                head,
                hazptr,
                // NOTE: Folly uses Release, but needs to be both for the
                // load on success.
                Ordering::AcqRel,
                Ordering::Acquire,
            ) {
                | Ok(_) => {
                    // NOTE: Folly uses SeqCst because it's the default,
                    // not clear if necessary.
                    self.hazptrs.count.fetch_add(1, Ordering::SeqCst);
                    // Safety: HazPtrRecords are never de-allocated while
                    // the domain lives.
                    break unsafe { &*hazptr };
                }
                | Err(head_now) => {
                    // Head has changed, try again with that as our next
                    // ptr.
                    head = head_now
                }
            }
    ...
    pub(crate) unsafe fn retire<'domain>(
        ...
        //
        // Safety: ptr will not be accessed after Domain is dropped, which
        // is when 'domain ends.
        let retired =
            Box::new(unsafe { Retired::new(self, ptr, deleter) });
    ...
    fn threshold(&self) -> isize {
        RCOUNT_THRESHOLD.max(
            HCOUNT_MULTIPLIER * self.hazptrs.count.load(Ordering::Acquire),
        )
    }
        ...
        while rcount > self.threshold() {
            match self.count.compare_exchange_weak(
                ...
                | Ok(_) => {
                    #[cfg(feature = "std")]
                    {
                        // We don't check `due_time` with no_std, so no
                        // need to set it either
                        self.due_time.store(
                            Self::now() + SYNC_TIME_PERIOD,
                            Ordering::Release,
                        );
                    }
                    return rcount;
    ...
    fn do_reclamation(&self, mut rcount: isize) -> usize {
        ...
        loop {
            ...
            if !empty {
                crate::asymmetric_heavy_barrier(
                    crate::HeavyBarrierKind::Expedited,
                );

                // Find all guarded addresses.
                #[allow(clippy::mutable_key_type)]
                //XXX: Maybe use a sorted vec to reduce heap allocations,
                // and have O(log(n)) lookups
                ...
                let (nreclaimed, is_done) = self
                    .match_reclaim_untagged(stolen_heads, &guarded_ptrs);
                done = is_done;
    ...
    fn match_reclaim_untagged(
        ...
        for i in 0..NUM_SHARDS {
            ...
            // XXX: This can probably also be hoisted out of the loop, and
            // we can do a _single_ reclaim_unprotected call as
            // well.
            let mut reclaimable = core::ptr::null_mut();

            while !node.is_null() {
                // Safety: All accessors only access the head, and the head
            ...
            // Safety:
            //
            // 1. No item in `reclaimable` has a hazard pointer guarding
            // it, so we have the    only remaining pointer to
            // each item. 2. Every Retired was originally
            // constructed from a Box, and is thus valid.
            // 3. None of these Retired have been dropped previously,
            // because we atomically    stole the entire
            // sublist from self.untagged.
            unsafe { self.reclaim_unprotected(reclaimable) };
        }

        let done = self.untagged.iter().all(|u| u.is_empty());
        // NOTE: We're _not_ respecting sharding here, presumably to avoid
        // multiple push CASes.
    ...
    // # Safety
    //
    // All `Retired` nodes in `retired` are valid, unaliased, and can be
    // taken ownership of.
    unsafe fn reclaim_unprotected(&self, mut retired: *mut Retired) {
        while !retired.is_null() {
            ...
            // Safety:
            //  - `n.ptr` has not yet been dropped because it was still on
            //    `retired`.
            //  - it will not be dropped again because we have removed it
            //    from `retired`.
            //  - `n.ptr` was allocated by the corresponding allocation
            //    method as per the safety guarantees of calling `retire`.
    ...
    fn reclaim_all_objects(&mut self) {
        for i in 0..NUM_SHARDS {
            ...
            // Safety: &mut self implies that there are no active Hazard
            // Pointers. So, all objects are safe to reclaim.
    ...
    /// Equivalent to reclaim_unprotected, but differs in name to clarify
    /// that it will remove indiscriminately.
    unsafe fn reclaim_unconditional(&self, head: *mut Retired) {
        unsafe { self.reclaim_unprotected(head) };
    }

    fn free_hazptr_recs(&mut self) {
        // NOTE: folly skips this step for the global domain, but the
        // global domain is never dropped in the first place, as it
        // is a static. See
        //
        //   https://doc.rust-lang.org/reference/items/static-items.html

        let mut node: *mut HazPtrRecord =
            self.hazptrs.head.with_mut(|p| *p);
        while !node.is_null() {
            // Safety: we have &mut self, so no-one holds any of our hazard
            // pointers any more, as all holders are tied to
            // 'domain (which must have expired to create the &mut).
...
struct Retired {
    // This is + 'domain, which is enforced for anything that constructs a
    // Retired
    ptr: *mut dyn Reclaim,
    deleter: &'static dyn Deleter,
...
impl Retired {
    ...
    unsafe fn new<'domain, F>(
        ...
        Retired {
            ptr: unsafe {
                core::mem::transmute::<_, *mut (dyn Reclaim + 'static)>(
                    ptr,
                )
            },
            deleter,
...
impl RetiredList {
    ...
    unsafe fn push(
        &self,
        sublist_head: *mut Retired,
        sublist_tail: *mut Retired,
    ) {
        ...
        loop {
            // Safety: we haven't moved anything in Retire, and we own the
            // head, so last_next is still valid.
            unsafe { &*sublist_tail }
                .next
...
mod tests {
    ...
    fn acquire_many_skips_used_nodes() {
        ...
        assert_eq!(
            rec1.next.load(Ordering::Relaxed),
            core::ptr::null_mut()
        );
        domain.release(rec1);
        domain.release(rec3);
        ...
        // so proper ordering for next is three -> rec3/one -> rec2 ->
        // rec1/two
        assert_eq!(
        ...
        assert_eq!(
            one.next.load(Ordering::Relaxed),
            rec2 as *const _ as *mut _
        );
        assert_eq!(
            rec2.next.load(Ordering::Relaxed),
    ...
    fn acquire_many_properly_orders_reused_nodes() {
        ...
        assert_eq!(
            four.next.load(Ordering::Relaxed),
            one as *const _ as *mut _
        );
        assert_eq!(
            one.next.load(Ordering::Relaxed),
